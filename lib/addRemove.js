 ...
 * occasional 'EPERM' or 'EACCES' errors.
 */
function renameWithRetry(from, to) {
	// Derived/simplified from https://github.com/isaacs/node-graceful-fs/pull/119
	let backoff = 0;
	...
	function tryRename() {
		try {
			...
			if (!isWindows) {
				...
				throw e;
			} else if ((e.code === 'EACCES' || e.code === 'EPERM') && Date.now() < backoffUntil) {
				if (backoff < 100) {
					backoff += 10;
				}
